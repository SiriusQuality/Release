...
namespace csMTG
{
    public class mtg : PropertyTree
    {
        ...
        public int AddComponent(int complexId, Dictionary<string, dynamic> namesValues = null, int componentId = -1)
        {
            ...
            if (scale.ContainsKey(componentId))
                return componentId;
            else
        ...
        public new int RemoveVertex(int vertexId, bool reparentChild = false)
        {
            if (reparentChild && NbComponents(vertexId) != 0)
            {
                int newParentId = (int)Parent(vertexId);
                List<int> children = new List<int>(Children(vertexId));
        ...
        public mtg FatMtg(mtg m, bool preOrd)
        {
            Algorithm a = new Algorithm();

            return a.FatMtg(m,preOrd);
        }
