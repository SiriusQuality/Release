...
!> $URL: https://biot.eos.ubc.ca/svn/min3p_thcm/branches/fgerard_new/src/solver/ws209.F90 $
!---------------------------------------------------------------------
!********************************************************************!
...
!c! Last Modified: March 3, 1996 (JVK)
     ...
     &          over_flow,rnorm,rmupdate,nthreads, b_output)
...
!culi
      ...
      IF (idetail.GT.i1 .and. b_output) THEN
        WRITE (lfile,9)
        WRITE (lfile,10)
        WRITE (lfile,11)
      ENDIF
...
!3    FORMAT (1x,i5,4x,3(2x,1pd11.4))      
      RETURN
      END
...
!c! Last Modified: March 3, 1996 (JVK)
     ...
     &          list,ia,ja,row,levptr,iaf,iafd,jaf,lorder,invord,b_output)
...
!c!provi------------------------------------------------------------------
      ...
      CALL Check (njafp1,mnjaf,lfile,b_output)       ! check dimensions
...
!c! Start scatter
        ...
        DO ii = istart,istop                   ! load row ir of L/U
          ...
          CALL Check (njaf + iend,mnjaf,lfile,b_output) ! check dimensions
          j = invord(ja(ii))
          jaf(iend) = j
...
!c! End merge, Start gather
        ...
        DO WHILE (next.LT.nnp1)             ! linked list loop (gather)
          njaf         = njaf + i1
          mmmm = max(njaf,mmmm)
          CALL Check (njaf,mnjaf,lfile,b_output)     ! check dimensions
...
!c! Last Modified: March 3, 1996 (JVK)
      ...
      SUBROUTINE Check (number,maximum,lfile,b_output)
