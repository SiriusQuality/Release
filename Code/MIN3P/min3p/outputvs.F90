...
!> $URL: https://biot.eos.ubc.ca/svn/min3p_thcm/branches/fgerard_new/src/min3p/outputvs.F90 $
!---------------------------------------------------------------------
!********************************************************************!
...
#include <finclude/petscsys.h>
      ...
      integer :: i, ivol, l_sufx, ivol_l, ierrcode !CBF, qroot
      
      real*8 :: theta_a, theta_g, rootwat, phead, zout, zoutput, evapo !CBF add
      external rootwat, evapo, velocity !CBF removed zout and zoutput, added evapo
	real*8 :: transp, soilevap ! CBF ADDED
	integer :: izn ! CBF
...
!c  title and variables                                                                       
  ...
  write(igsp,'(12e15.7)') xg(ivol),yg(ivol),zout,hhead(ivol),&
     &                              uvsnew(ivol),sanew(ivol),theta_a,  &
     &                              sgnew(ivol),theta_g, &
     &                              transp, soilevap, &!FG
     &                              alphafeddes        !Mujica 2022
...
!c  compute root uptake rate
		...
		izn = mpropvs(ivol)	! CBF
	
	    transp = cvol(ivol) * rootwat(sanew,ivol)/sec_per_days	! CBF

	
		if (h1dry(izn).gt.r0) then! CBF
		
               		soilevap = cvol(ivol) * evapo(sanew,ivol)	&! CBF
    		 &         /sec_per_days! CBF
...
!c  assign depth coordinate in terms of depth or elevation
        ...
        if (fully_saturated) then
          ...
          if (b_output_binary) then
            ...
            write(igsp,'(7e15.7)') xg(ivol),yg(ivol),zout,hhead(ivol), &
...
! CBF B4
!         write(igsp,'(10e15.7)') xg(ivol),yg(ivol),zout,hhead(ivol),&
!     &                              uvsnew(ivol),sanew(ivol),theta_a,  &
...
!                pause
     ...
     &                 nn, half_cells,cinfrad,radial_coord, offset_ivel)
