...
namespace SiriusQualityEnergyBalance
{
	...
	/// <summary>
	///Class CalculateSoilHeatFlux
    /// 
    /// </summary>
	public class CalculateSoilHeatFlux : IStrategySiriusQualityEnergyBalance
	{

	#region Constructor

			public CalculateSoilHeatFlux()
			{
				...
				pd4.PropertyName = "soilHeatFlux";
				pd4.PropertyType =  (( SiriusQualityEnergyBalance.EnergyBalanceStateVarInfo.soilHeatFlux)).ValueType.TypeForCurrentValue;
				pd4.PropertyVarInfo =(  SiriusQualityEnergyBalance.EnergyBalanceStateVarInfo.soilHeatFlux);
				_outputs0_0.Add(pd4);
	...
	#region pre/post conditions management		
			...
			public string TestPostConditions(SiriusQualityEnergyBalance.EnergyBalanceState energybalancestate, string callID)
			{
				try
				{
					...
					RangeBasedCondition r4 = new RangeBasedCondition(SiriusQualityEnergyBalance.EnergyBalanceStateVarInfo.soilHeatFlux);
					if(r4.ApplicableVarInfoValueTypes.Contains( SiriusQualityEnergyBalance.EnergyBalanceStateVarInfo.soilHeatFlux.ValueType)){prc.AddCondition(r4);}
			...
			public string TestPreConditions(SiriusQualityEnergyBalance.EnergyBalanceState energybalancestate, string callID)
			{
				try
				{
					...
					if(r1.ApplicableVarInfoValueTypes.Contains( SiriusQualityEnergyBalance.EnergyBalanceStateVarInfo.netRadiationEquivalentEvaporation.ValueType)){prc.AddCondition(r1);}
	...
	#region Model
			...
			{				
				...
				//Code written below will not be overwritten by a future code generation
                energybalancestate.soilHeatFlux = energybalancestate.tau * energybalancestate.netRadiationEquivalentEvaporation - energybalancestate.soilEvaporation;
