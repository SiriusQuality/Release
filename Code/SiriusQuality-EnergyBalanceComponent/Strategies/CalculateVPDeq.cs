...
namespace SiriusQualityEnergyBalance
{
	...
	/// <summary>
	///Class calculateVPDeq
    /// 
    /// </summary>
	public class CalculateVPDeq : IStrategySiriusQualityEnergyBalance
	{

	#region Constructor

			public CalculateVPDeq()
			{
				...
				pd1.PropertyName = "hourlySolarRadiation";
				pd1.PropertyType = (( SiriusQualityEnergyBalance.EnergyBalanceStateVarInfo.hourlySolarRadiation)).ValueType.TypeForCurrentValue;
				pd1.PropertyVarInfo =( SiriusQualityEnergyBalance.EnergyBalanceStateVarInfo.hourlySolarRadiation);
				_inputs0_0.Add(pd1);
				PropertyDescription pd2 = new PropertyDescription();
				pd2.DomainClassType = typeof(SiriusQualityEnergyBalance.EnergyBalanceState);
				pd2.PropertyName = "hourlyVPDairLeaf";
				pd2.PropertyType = (( SiriusQualityEnergyBalance.EnergyBalanceStateVarInfo.hourlyVPDairLeaf)).ValueType.TypeForCurrentValue;
				pd2.PropertyVarInfo =( SiriusQualityEnergyBalance.EnergyBalanceStateVarInfo.hourlyVPDairLeaf);
				_inputs0_0.Add(pd2);
				...
				pd3.PropertyType =  (( SiriusQualityEnergyBalance.EnergyBalanceStateVarInfo.VPDeq)).ValueType.TypeForCurrentValue;
				pd3.PropertyVarInfo =(  SiriusQualityEnergyBalance.EnergyBalanceStateVarInfo.VPDeq);
				_outputs0_0.Add(pd3);
	...
	#region pre/post conditions management		
			...
			public string TestPostConditions(SiriusQualityEnergyBalance.EnergyBalanceState energybalancestate, string callID)
			{
				try
				{
					...
					SiriusQualityEnergyBalance.EnergyBalanceStateVarInfo.VPDeq.CurrentValue=energybalancestate.VPDeq;
					...
					RangeBasedCondition r3 = new RangeBasedCondition(SiriusQualityEnergyBalance.EnergyBalanceStateVarInfo.VPDeq);
					if(r3.ApplicableVarInfoValueTypes.Contains( SiriusQualityEnergyBalance.EnergyBalanceStateVarInfo.VPDeq.ValueType)){prc.AddCondition(r3);}
			...
			public string TestPreConditions(SiriusQualityEnergyBalance.EnergyBalanceState energybalancestate, string callID)
			{
				try
				{
					...
					RangeBasedCondition r1 = new RangeBasedCondition(SiriusQualityEnergyBalance.EnergyBalanceStateVarInfo.hourlySolarRadiation);
					if(r1.ApplicableVarInfoValueTypes.Contains( SiriusQualityEnergyBalance.EnergyBalanceStateVarInfo.hourlySolarRadiation.ValueType)){prc.AddCondition(r1);}
					RangeBasedCondition r2 = new RangeBasedCondition(SiriusQualityEnergyBalance.EnergyBalanceStateVarInfo.hourlyVPDairLeaf);
					if(r2.ApplicableVarInfoValueTypes.Contains( SiriusQualityEnergyBalance.EnergyBalanceStateVarInfo.hourlyVPDairLeaf.ValueType)){prc.AddCondition(r2);}
	...
	#region Model
			...
			{				
                ...
                for (int i = 0; i < 24; i++)
                {
                    double PAR = (energybalancestate.hourlySolarRadiation[i] * 2.02 / 3600 * 1e6);
                    energybalancestate.VPDeq[i] = (energybalancestate.hourlyVPDairLeaf[i] * Math.Max(0.0, Math.Min(PAR / 500.0, 1.0)));
                }
