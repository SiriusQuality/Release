...
namespace SiriusQualityEnergyBalance
{
	...
	/// <summary>
	///Class CalculateDiffusionLimitedEvaporation
    /// 
    /// </summary>
	public class CalculateDiffusionLimitedEvaporation : IStrategySiriusQualityEnergyBalance
	{

	#region Constructor

			public CalculateDiffusionLimitedEvaporation()
			{
				...
				VarInfo v1 = new VarInfo();
				 v1.DefaultValue = 4.2;
				 v1.Description = "soilDiffusionConstant";
				 v1.Id = 0;
				 v1.MaxValue = 10;
				 v1.MinValue = 0;
				...
				pd1.PropertyType = (( SiriusQualityEnergyBalance.EnergyBalanceStateVarInfo.deficitOnTopLayers)).ValueType.TypeForCurrentValue;
				pd1.PropertyVarInfo =( SiriusQualityEnergyBalance.EnergyBalanceStateVarInfo.deficitOnTopLayers);
				_inputs0_0.Add(pd1);
    ...
    #region Instances of the parameters
			...
			{ 
				get {
						...
						else throw new Exception("Parameter 'soilDiffusionConstant' not found (or found null) in strategy 'CalculateDiffusionLimitedEvaporation'");
				 } set {
							VarInfo vi = _modellingOptionsManager.GetParameterByName("soilDiffusionConstant");
							if (vi != null)  vi.CurrentValue=value;
						else throw new Exception("Parameter 'soilDiffusionConstant' not found in strategy 'CalculateDiffusionLimitedEvaporation'");
				}
			}
	...
	#region Static parameters VarInfo definition
			...
			{                                
				...
				soilDiffusionConstantVarInfo.Description =" soilDiffusionConstant";
				...
				soilDiffusionConstantVarInfo.ValueType = CRA.ModelLayer.Core.VarInfoValueTypes.GetInstanceForName("Double");
	...
	#region pre/post conditions management		
			...
			public string TestPostConditions(SiriusQualityEnergyBalance.EnergyBalanceState energybalancestate, string callID)
			{
				try
				{
					...
					RangeBasedCondition r2 = new RangeBasedCondition(SiriusQualityEnergyBalance.EnergyBalanceStateVarInfo.diffusionLimitedEvaporation);
					if(r2.ApplicableVarInfoValueTypes.Contains( SiriusQualityEnergyBalance.EnergyBalanceStateVarInfo.diffusionLimitedEvaporation.ValueType)){prc.AddCondition(r2);}
			...
			public string TestPreConditions(SiriusQualityEnergyBalance.EnergyBalanceState energybalancestate, string callID)
			{
				try
				{
					...
					SiriusQualityEnergyBalance.EnergyBalanceStateVarInfo.deficitOnTopLayers.CurrentValue=energybalancestate.deficitOnTopLayers;
					...
					RangeBasedCondition r1 = new RangeBasedCondition(SiriusQualityEnergyBalance.EnergyBalanceStateVarInfo.deficitOnTopLayers);
					if(r1.ApplicableVarInfoValueTypes.Contains( SiriusQualityEnergyBalance.EnergyBalanceStateVarInfo.deficitOnTopLayers.ValueType)){prc.AddCondition(r1);}
	...
	#region Model
			...
			{				
				...
				//Code written below will not be overwritten by a future code generation
                if (energybalancestate.deficitOnTopLayers / 1000 <= 0)
                { 
                    energybalancestate.diffusionLimitedEvaporation = 8.3 * 1000;
                }
                else
                {
                    if (energybalancestate.deficitOnTopLayers / 1000 < 25)
                    {
                        energybalancestate.diffusionLimitedEvaporation = (2 * soilDiffusionConstant * soilDiffusionConstant / (energybalancestate.deficitOnTopLayers / 1000)) * 1000;
                    }
                    else
                    {
                        energybalancestate.diffusionLimitedEvaporation = 0;
                    }
                }
